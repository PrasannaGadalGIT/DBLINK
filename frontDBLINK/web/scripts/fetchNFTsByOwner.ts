...
(async () => {
  ...
  if (!keys?.reciever) return console.warn("No local keys were found, specifically `reciever`");

  // convert the locally saved keys to PublicKeys
  const treeAddress: PublicKey = keys.treeAddress;
  const reciever: PublicKey = keys.reciever;
  const userAddress: PublicKey = keys.userAddress;

  console.log("==== Local PublicKeys loaded ====");
  console.log("Test address:", reciever.toBase58());
  ...
  printConsoleSeparator(`Getting all assets by owner ${reciever.toBase58()}...`);

  await connection
    .getAssetsByOwner({
      ownerAddress: reciever.toBase58(),
    })
    .then(res => {
