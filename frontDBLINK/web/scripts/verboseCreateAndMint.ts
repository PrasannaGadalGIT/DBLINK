 ...
 * Compressed NFTs on Solana, using State Compression
  ...
  - load or create two keypairs (named `payer` and `reciever`)
  ...
  - mint a single compressed nft into the tree to the `reciever`
...
import { CreateMetadataAccountArgsV3, DataV2 } from "@metaplex-foundation/mpl-token-metadata";
...
export default async function CreateandMintNFT( receiverWalletAddress : PublicKey) {
  ...
  const reciever = receiverWalletAddress;
  ...
  */
  ...
  console.log("NFT metadata  : "  + collectionMetadataV3.data.uri)
  ...
  */
  ...
  await mintCompressedNFT(
    ...
    collection.masterEditionAccount,
    compressedNFTMetadata,
    // mint to this specific wallet (in this case, airdrop to `reciever`)
    reciever,
  );
